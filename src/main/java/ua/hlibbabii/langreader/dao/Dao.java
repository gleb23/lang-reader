...
public class Dao {
    ...
    @PostConstruct
    public void init() {
        textViewSimpleJdbcInsert = new SimpleJdbcInsert(jdbcTemplate)
                .withTableName("TEXT_VIEW")
                .usingColumns("textId", "userId", "dateTime")
                .usingGeneratedKeyColumns("textViewId");

        unknownWordOcurrenceJdbcInsert = new SimpleJdbcInsert(jdbcTemplate)
                .withTableName("UNKNOWN_WORD_OCURRENCE")
                .usingColumns("userId", "textViewId", "normalizedForm");
    }

    public void addWordOccurence(int userId, String textViewId, String normalizedForm) {
        unknownWordOcurrenceJdbcInsert
    ...
    public int addTextView(String textId, int userId, Date timestamp) {
        return textViewSimpleJdbcInsert
    ...
    public Map<String, List<Integer>> getAllUnknownWordsWithTextViews(int userId) {
        ...
        String sql = "SELECT w.normalizedForm, t.textViewId from UNKNOWN_WORD_OCURRENCE as w INNER JOIN TEXT_VIEW as t on w.textViewId = t.textViewId inner join USER as u on u.userId = t.userId where u.userId = ?";
    ...
    public Map<Integer, TextView> getAllTextViewsForUser(int userId) {
        ...
        List<TextView> textViews  = jdbcTemplate.query(sql,
                new Object[]{userId},
                new int[]{Types.INTEGER}, (rs, rowNum) -> {
        ...
        Map<Integer, TextView> result = textViews.stream().collect(
                Collectors.toMap(tw -> tw.getTextViewId(), tw -> tw));
        return result;
    }
}
